...
def parser_args_for_sac():
    ...
    parser.add_argument('--baseline_model', '-bm', type=str,
                        ...
                        required=False, help='path to linear regression')
...
if __name__ == '__main__':
    ...
    cat_model = GridSearchCV(CatBoostRegressor(), params['grid_search'])
    ...
    predicted_values = np.squeeze(cat_model.predict(X_test))
    ...
    # feature_importance = cat_model.best_estimator_.get_feature_importance()
    # feature_names = X_train.columns
    # plt.figure(figsize=(10, 6))
    # plt.xlim(0,80)
    # bar = sns.barplot(x=feature_importance, y=feature_names, width=0.6)
    ...
    #     bar.bar_label(i,)
    # plt.grid()
    # plt.savefig('images/catbFeatImp.png')
    # plt.show()

    dump(cat_model, output_model_joblib_path)
